...
SELECT MIN(released_year) FROM gbooks;

/*Find the longest book (but took long as 2 quries have to run)*/
SELECT *
FROM gbooks
WHERE pages = (SELECT MAX(pages) FROM gbooks);
...
SELECT AVG(released_year) FROM gbooks;
...
SELECT COUNT(*) AS 'number of books' FROM gbooks;
